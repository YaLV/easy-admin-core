        ...
        <div class="sv-product-card {{ $item->isSale() }}">
            ...
            <div class="text">
                ...
                @if($item->hasManyPrices())
                    <select name="variation[{{$item->id}}]" class="selectpicker">
                        @foreach($item->prices() as $priceID => $price)
                            <option value="{{ $priceID }}" {{$loop->first?"selected":""}} {{ $item->isSale()?"data-origprice={$price->oldPrice}€":"" }}>{{ implode(" / ",[$price->price."€", $price->display_name]) }}</option>
                        @endforeach
                    </select>
                @else
                    <h3>
